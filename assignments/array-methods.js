...
largeShirts = runners.filter(largeShirt => {
  if (largeShirt.shirt_size === "L") 
    return true;
...
idLastName = runners.map( runner => {
  return runner.id + " " + runner.last_name;
...
xtraLarge = runners.filter( xtra => { 
	if (xtra.shirt_size < "L") 
		return true;	
